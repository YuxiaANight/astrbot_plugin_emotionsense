...
- 「刷新数据」按钮重新拉取最新情绪数据；「清除所有数据」按钮会在二次确认后清空全部记录。
- 表格列出每个用户的 ID（截断显示）、当前情绪、情绪强度百分比与历史记录数。
- 窄屏（手机）下统计卡片与按钮自动两列排列，表格可横向滚动。
...
- `GET /astrbot_plugin_emotionsense/data`：返回全部用户的情绪数据。
- `POST /astrbot_plugin_emotionsense/clear`：清空全部情绪数据。

## 依赖

- AstrBot（建议 v4.13.0 及以上，需支持 `astrbot.api.web`、插件 KV 存储与 Pages）。
